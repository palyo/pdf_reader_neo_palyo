...
public class LineView extends AbstractView {
    ...
    public void draw(Canvas canvas, int originX, int originY, float zoom) {
        canvas.save();
        IWord word = getContainer();
        int dX = (int) (x * zoom) + originX;
        int dY = (int) (y * zoom) + originY;
        IView view = getChildView();
        Rect clip = canvas.getClipBounds();
        if (getTopIndent() < 0 && word != null && word.getEditType() == KeyKt.APPLICATION_TYPE_WP) {
            canvas.clipRect(dX, dY - getTopIndent() * zoom, dX + getLayoutSpan(WPViewConstant.X_AXIS) * zoom, dY - getTopIndent() * zoom + getLayoutSpan(WPViewConstant.Y_AXIS) * zoom);
        }
        while (view != null) {
