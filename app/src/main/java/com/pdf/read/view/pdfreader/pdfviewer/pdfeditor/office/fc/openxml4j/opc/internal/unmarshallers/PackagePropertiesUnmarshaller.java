...
import com.pdf.read.view.pdfreader.pdfviewer.pdfeditor.office.fc.dom4j.DocumentException;
...
import com.pdf.read.view.pdfreader.pdfviewer.pdfeditor.office.fc.openxml4j.opc.PackageNamespaces;
    ...
    public PackagePart unmarshall(UnmarshallContext context, InputStream in)
        ...
        if (in == null) {
            if (context.getZipEntry() != null) {
                in = context.getPackage().getZipArchive().getInputStream(
                        context.getZipEntry());
            } else if (context.getPackage() != null) {

                ZipEntry zipEntry = ZipHelper.getCorePropertiesZipEntry(context
                        .getPackage());
                in = context.getPackage().getZipArchive().getInputStream(zipEntry);
            } else {
