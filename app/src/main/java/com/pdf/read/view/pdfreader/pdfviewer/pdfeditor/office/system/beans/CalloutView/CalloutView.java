...
public class CalloutView extends View {
    ...
    private final CalloutManager calloutMgr;
    ...
    private void touch_start(float x, float y) {
        ...
        if (calloutMgr.getDrawingMode() == KeyKt.DRAW_MODE_CALL_OUT_DRAW) {
    ...
    private void touch_up() {
        if (calloutMgr.getDrawingMode() == KeyKt.DRAW_MODE_CALL_OUT_DRAW) {
            ...
            mPathInfo.x = mX + 1;
            mPathInfo.y = mY + 1;
        } else if (calloutMgr.getDrawingMode() == KeyKt.DRAW_MODE_CALL_OUT_ERASE) {
            if (mPathList != null) {
                for (int i = 0; i < mPathList.size(); i++) {
                    PathInfo pathInfo = mPathList.get(i);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (calloutMgr.getDrawingMode() == KeyKt.DRAW_MODE_NORMAL) {
            return false;
        }
        float x = event.getX();
        float y = event.getY();
