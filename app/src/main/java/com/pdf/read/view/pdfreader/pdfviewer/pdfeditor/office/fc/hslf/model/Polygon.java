...
public final class Polygon extends AutoShape {
    private Polygon(EscherContainerRecord escherRecord, Shape parent) {
    ...
    public void setPoints(float[] xPoints, float[] yPoints) {
        ...
        opt.addEscherProperty(new EscherSimpleProperty(EscherProperties.GEOMETRY__RIGHT,
                (int) ((right - left) * KeyKt.POINT_DPI / ShapeKit.MASTER_DPI)));
        opt.addEscherProperty(new EscherSimpleProperty(EscherProperties.GEOMETRY__BOTTOM,
                (int) ((bottom - top) * KeyKt.POINT_DPI / ShapeKit.MASTER_DPI)));
        ...
        for (int i = 0; i < numpoints; i++) {
            ...
            LittleEndian.putShort(data, 0, (short) (xPoints[i] * KeyKt.POINT_DPI / ShapeKit.MASTER_DPI));
            LittleEndian.putShort(data, 2, (short) (yPoints[i] * KeyKt.POINT_DPI / ShapeKit.MASTER_DPI));
            verticesProp.setElement(i, data);
        }
        byte[] data = new byte[4];
        LittleEndian.putShort(data, 0, (short) (xPoints[0] * KeyKt.POINT_DPI / ShapeKit.MASTER_DPI));
        LittleEndian.putShort(data, 2, (short) (yPoints[0] * KeyKt.POINT_DPI / ShapeKit.MASTER_DPI));
