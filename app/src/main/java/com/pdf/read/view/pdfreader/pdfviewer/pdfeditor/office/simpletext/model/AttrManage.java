...
import com.pdf.read.view.pdfreader.pdfviewer.pdfeditor.office.simpletext.view.CharAttr;
...
public class AttrManage {
    ...
    public void fillPageAttr(PageAttr pageAttr, IAttributeSet attr) {
        ...
        pageAttr.pageWidth = (int) (getPageWidth(attr) * KeyKt.TWIPS_TO_PIXEL);
        pageAttr.pageHeight = (int) (getPageHeight(attr) * KeyKt.TWIPS_TO_PIXEL);
        pageAttr.topMargin = (int) (getPageMarginTop(attr) * KeyKt.TWIPS_TO_PIXEL);
        pageAttr.bottomMargin = (int) (getPageMarginBottom(attr) * KeyKt.TWIPS_TO_PIXEL);
        pageAttr.rightMargin = (int) (getPageMarginRight(attr) * KeyKt.TWIPS_TO_PIXEL);
        pageAttr.leftMargin = (int) (getPageMarginLeft(attr) * KeyKt.TWIPS_TO_PIXEL);
        pageAttr.headerMargin = (int) (getPageHeaderMargin(attr) * KeyKt.TWIPS_TO_PIXEL);
        pageAttr.footerMargin = (int) (getPageFooterMargin(attr) * KeyKt.TWIPS_TO_PIXEL);

        pageAttr.pageBRColor = getPageBackgroundColor(attr);
        pageAttr.pageBorder = getPageBorder(attr);
        pageAttr.pageLinePitch = (getPageLinePitch(attr) * KeyKt.TWIPS_TO_PIXEL);
    }

    public void fillParaAttr(IControl control, ParaAttr paraAttr, IAttributeSet attr) {
        paraAttr.reset();
        paraAttr.tabClearPosition = (int) (getParaTabsClearPostion(attr) * KeyKt.TWIPS_TO_PIXEL);
        paraAttr.leftIndent = (int) (getParaIndentLeft(attr) * KeyKt.TWIPS_TO_PIXEL);
        paraAttr.rightIndent = (int) (getParaIndentRight(attr) * KeyKt.TWIPS_TO_PIXEL);
        paraAttr.beforeSpace = (int) (getParaBefore(attr) * KeyKt.TWIPS_TO_PIXEL);
        paraAttr.afterSpace = (int) (getParaAfter(attr) * KeyKt.TWIPS_TO_PIXEL);
        ...
        if (paraAttr.lineSpaceType == WPAttrConstant.LINE_SAPCE_LEAST
            ...
            paraAttr.lineSpaceValue *= KeyKt.TWIPS_TO_PIXEL;
        }

        paraAttr.horizontalAlignment = (byte) getParaHorizontalAlign(attr);

        paraAttr.specialIndentValue = (int) (getParaSpecialIndent(attr) * KeyKt.TWIPS_TO_PIXEL);
        ...
        if (paraAttr.listID >= 0 && paraAttr.listLevel >= 0 && control != null) {
            ...
            if (listData != null) {
                ...
                if (listLevel != null) {
                    ...
                    paraAttr.listTextIndent = (int) (listLevel.getTextIndent() * KeyKt.TWIPS_TO_PIXEL);

                    paraAttr.listAlignIndent = paraAttr.listTextIndent + (int) (listLevel.getSpecialIndent() * KeyKt.TWIPS_TO_PIXEL);
    ...
    public void fillTableAttr(TableAttr tableAttr, IAttributeSet attr) {
        ...
        tableAttr.cellWidth = (int) (getTableCellWidth(attr) * KeyKt.TWIPS_TO_PIXEL);
