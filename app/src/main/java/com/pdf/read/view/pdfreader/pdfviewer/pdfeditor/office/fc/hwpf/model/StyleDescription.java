...
import com.pdf.read.view.pdfreader.pdfviewer.pdfeditor.office.fc.hwpf.usermodel.CharacterProperties;
import com.pdf.read.view.pdfreader.pdfviewer.pdfeditor.office.fc.hwpf.usermodel.ParagraphProperties;
...
public final class StyleDescription implements HDFType {
    ...
    private static final BitField _sti = BitFieldFactory.getInstance(0xfff);
    private static final BitField _fScratch = BitFieldFactory.getInstance(0x1000);
    private static final BitField _fInvalHeight = BitFieldFactory.getInstance(0x2000);
    private static final BitField _fHasUpe = BitFieldFactory.getInstance(0x4000);
    private static final BitField _fMassCopy = BitFieldFactory.getInstance(0x8000);
    private static final BitField _styleTypeCode = BitFieldFactory.getInstance(0xf);
    private static final BitField _baseStyle = BitFieldFactory.getInstance(0xfff0);
    private static final BitField _numUPX = BitFieldFactory.getInstance(0xf);
    private static final BitField _nextStyle = BitFieldFactory.getInstance(0xfff0);
    private static final BitField _fAutoRedef = BitFieldFactory.getInstance(0x1);
    private static final BitField _fHidden = BitFieldFactory.getInstance(0x2);
    UPX[] _upxs;
    String _name;
    ...
    public StyleDescription(byte[] std, int baseLength, int offset, boolean word9) {
        ...
        _name = new String(std, nameStart, nameLength * multiplier, StandardCharsets.UTF_16LE);
    ...
    public boolean equals(Object o) {
        ...
        if (sd._infoShort == _infoShort && sd._infoShort2 == _infoShort2 &&
            ...
            return Arrays.equals(_upxs, sd._upxs);
        }
        return false;
    }
}
