package com.pdf.read.view.pdfreader.pdfviewer.pdfeditor.office.fc.xls.Reader;
...
import com.pdf.read.view.pdfreader.pdfviewer.pdfeditor.office.fc.xls.Reader.drawing.DrawingReader;
import com.pdf.read.view.pdfreader.pdfviewer.pdfeditor.office.fc.xls.Reader.table.TableReader;
...
import com.pdf.read.view.pdfreader.pdfviewer.pdfeditor.office.ss.model.sheetProperty.ColumnInfo;
...
import com.pdf.read.view.pdfreader.pdfviewer.pdfeditor.office.system.IReader;
...
public class SheetReader {
    ...
    public void getSheet(IControl control, ZipPackage zipPackage, Sheet sheet, PackagePart sheetPart, IReader iReader) throws Exception {
        ...
        PackageRelationshipCollection tableRelCollection = sheetPart.getRelationshipsByType(PackageRelationshipTypes.TABLE_PART);
        ...
        PackageRelationshipCollection drawingRelCollection = sheetPart.getRelationshipsByType(PackageRelationshipTypes.DRAWING_PART);
    ...
    private Map<String, String> getSheetHyperlinkByRelation(PackagePart sheetPart) throws Exception {
        ...
        PackageRelationshipCollection HyperlinkRelCollection = sheetPart.getRelationshipsByType(PackageRelationshipTypes.HYPERLINK_PART);

        Map<String, String> hyperlink = new HashMap<String, String>(HyperlinkRelCollection.size());
    ...
    private void getSheetHyperlink(Sheet sheet, Map<String, String> hyperlinkTarget, Element hyperlinksEle) {
        ...
        @SuppressWarnings("unchecked") Iterator<Element> iter = hyperlinksEle.elementIterator();
    ...
    private void setColumnProperty(Element col) {
        ...
        if (col.attributeValue("width") != null) {
            width = Double.parseDouble(col.attributeValue("width")) * SSConstant.COLUMN_CHAR_WIDTH * KeyKt.POINT_TO_PIXEL;
        } else {
            width = 0;
        }

        if (col.attributeValue("hidden") != null) {
            if (col.attributeValue("hidden").equals("false")) {
                hidden = false;
            } else if (col.attributeValue("hidden").equals("true")) {
                hidden = true;
            } else {
    ...
    private void getSheetMergerdCells(Element mergedCell) {
        ...
        if (cellRange.getLastRow() - cellRange.getFirstRow() == Workbook.MAXROW_07 - 1 || cellRange.getLastColumn() - cellRange.getFirstColumn() == Workbook.MAXCOLUMN_07 - 1) {
            return;
        }
    ...
    private Row createRow(Element rowElement, int defaultRowHeight) {
        ...
        if (rowElement.attributeValue("ht") != null) {
            height = (Float.parseFloat(rowElement.attributeValue("ht")) * KeyKt.POINT_TO_PIXEL);
        }
    ...
    private void modifyRow(Row row, Element rowElement, int defaultRowHeight) {
        ...
        if (rowElement.attributeValue("ht") != null) {
            height = (int) (Double.parseDouble(rowElement.attributeValue("ht")) * KeyKt.POINT_TO_PIXEL);
        }

        if (rowElement.attributeValue("hidden") != null) {
            if (rowElement.attributeValue("hidden").equals("false")) {
                hidden = false;
            } else if (rowElement.attributeValue("hidden").equals("true")) {
                hidden = true;
            } else {
                hidden = (Integer.parseInt(rowElement.attributeValue("hidden")) != 0);
            }
        ...
        public void onEnd(ElementPath elementPath) {
            ...
            if (name.equals("sheetFormatPr")) {
                if (elem.attributeValue("defaultRowHeight") != null) {
                    defaultRowHeight = (int) (Double.parseDouble(elem.attributeValue("defaultRowHeight")) * KeyKt.POINT_TO_PIXEL);

                    sheet.setDefaultRowHeight(defaultRowHeight);
                }

                if (elem.attributeValue("defaultColWidth") != null) {
                    defaultColWidth = (int) (Double.parseDouble(elem.attributeValue("defaultColWidth")) * SSConstant.COLUMN_CHAR_WIDTH * KeyKt.POINT_TO_PIXEL);
