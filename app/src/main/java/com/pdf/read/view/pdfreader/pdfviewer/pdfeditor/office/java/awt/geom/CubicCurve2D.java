...
public abstract class CubicCurve2D implements Shape, Cloneable {
    ...
    public static double getFlatnessSq(double[] coords, int offset) {
        return getFlatnessSq(coords[offset], coords[offset + 1], coords[offset + 2],
                coords[offset + 3], coords[offset + 4], coords[offset + 5], coords[offset + 6],
                coords[offset + 7]);
    }

    public static double getFlatness(double[] coords, int offset) {
        return getFlatness(coords[offset], coords[offset + 1], coords[offset + 2],
                coords[offset + 3], coords[offset + 4], coords[offset + 5], coords[offset + 6],
                coords[offset + 7]);
    }
    ...
    private static void fillEqn(double[] eqn, double val, double c1, double cp1, double cp2,
                                double c2) {
        eqn[0] = c1 - val;
        eqn[1] = (cp1 - c1) * 3.0;
    ...
    private static int evalCubic(double[] vals, int num, boolean include0, boolean include1,
                                 double[] inflect, double c1, double cp1, double cp2, double c2) {
        int j = 0;
        for (int i = 0; i < num; i++) {
            double t = vals[i];
