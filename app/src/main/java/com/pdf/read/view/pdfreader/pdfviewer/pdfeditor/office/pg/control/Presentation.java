...
import com.pdf.read.view.pdfreader.pdfviewer.pdfeditor.office.pg.view.SlideShowView;
...
import com.pdf.read.view.pdfreader.pdfviewer.pdfeditor.office.system.beans.CalloutView.CalloutView;
import com.pdf.read.view.pdfreader.pdfviewer.pdfeditor.office.system.beans.CalloutView.IExportListener;
...
public class Presentation extends FrameLayout implements IFind, IExportListener {
    ...
    private final PGPrintMode pgPrintMode;
    ...
    public void beginSlideShow(int slideIndex) {
        synchronized (this) {
            ...
            boolean isChangedSlide = getCurrentIndex() + 1 != slideIndex;

            setOnTouchListener(eventManage);

            control.getSysKit().getCalloutManager().setDrawingMode(KeyKt.DRAW_MODE_NORMAL);
    ...
    public boolean hasNextAction_Slideshow() {
        synchronized (this) {
            if (slideshow) {
                ...
                return !slideView.gotoNextSlide()
    ...
    public void slideShow(byte type) {
        synchronized (this) {
            if (!slideshow || !slideView.animationStoped()
                    || control.getSysKit().getCalloutManager().getDrawingMode() != KeyKt.DRAW_MODE_NORMAL) {
                return;
            }
