package com.pdf.read.view.pdfreader.pdfviewer.pdfeditor.office.common.autoshape.pathbuilder.wedgecallout;
...
public class WedgeCalloutDrawing {
    ...
    private static Path getWedgeRectCalloutPath(AutoShape shape, Rect rect) {
        ...
        if (Math.abs(y / x) < (float) rect.height() / rect.width()) {
    ...
    private static Path getBorderCallout1Path(AutoShape shape, Rect rect) {
        ...
        path.addRect(rect.left, rect.top, rect.right, rect.bottom, Direction.CW);
