...
import com.pdf.read.view.pdfreader.pdfviewer.pdfeditor.office.fc.hslf.model.textproperties.AlignmentTextProp;
...
public final class TxMasterStyleAtom extends RecordAtom {
    ...
    private static final long _type = 4003;
    private byte[] _header;
    private byte[] _data;
    private TextPropCollection[] prstyles;
    private TextPropCollection[] chstyles;

    private TxMasterStyleAtom(byte[] source, int start, int len) {
        _header = new byte[8];
    ...
    private TextProp[] getParagraphProps(int type, int level) {
    ...
    private TextProp[] getCharacterProps(int type, int level) {
        if (level != 0 || type >= MAX_INDENT) {
