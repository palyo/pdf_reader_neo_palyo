package com.pdf.read.view.pdfreader.pdfviewer.pdfeditor.office.fc.ppt.attribute;
...
public class ParaAttr {
    ...
    private static final ParaAttr kit = new ParaAttr();
    ...
    public int processParagraph(IControl control, PGMaster master, PGLayout pgLayout, PGStyle defaultStyle,
        ...
        for (Element p : ps) {
            ...
            setParaAttribute(control, pPr, paraElem.getAttribute(), attrLayout, layoutStyle, masterStyle,
                    lnSpcReduction, p.elements("r").size() != 0 || p.elements("fld").size() != 0, subTitle);
    ...
    public void setParaAttribute(IControl control, Element pPr, IAttributeSet attr, IAttributeSet attrLayout,
                ...
                if (temp != null && temp.attribute("val") != null) {
                    ...
                    if (val != null && val.length() > 0) {
                        AttrManage.instance().setParaBefore(attr,
                                (int) (Integer.parseInt(val) / 100 * KeyKt.POINT_TO_TWIPS));
                    }
                }
            } else {
            ...
            if (pPr.attribute("marR") != null) {
                ...
                if (val != null && val.length() > 0) {
                    AttrManage.instance().setParaIndentRight(attr,
                            (int) (Integer.parseInt(val) * KeyKt.POINT_DPI / KeyKt.EMU_PER_INCH
                                    * KeyKt.POINT_TO_TWIPS));
                }
            } else {
        ...
        if (pPr != null && pPr.attribute("marL") != null) {
            ...
            if (val != null && val.length() > 0) {
                left = (int) (Integer.parseInt(val) * KeyKt.POINT_DPI / KeyKt.EMU_PER_INCH
                        * KeyKt.POINT_TO_TWIPS);
        ...
        if (pPr != null && pPr.attribute("indent") != null) {
            ...
            if (val != null && val.length() > 0) {
                indent = (int) (Integer.parseInt(val) * KeyKt.POINT_DPI / KeyKt.EMU_PER_INCH
                        * KeyKt.POINT_TO_TWIPS);
    ...
    public void setParaAttribute(CellStyle style, IAttributeSet attr, IAttributeSet attrLayout) {
        if (style != null && attrLayout != null) {
            int indent = style.getIndent() * SSConstant.INDENT_TO_PIXEL;

            switch (style.getHorizontalAlign()) {
                case CellStyle.ALIGN_LEFT:
                case CellStyle.ALIGN_GENERAL:
                    attrLayout.setAttribute(AttrIDConstant.PARA_INDENT_LEFT_ID, Math.round(indent * KeyKt.PIXEL_TO_TWIPS));
                ...
                case CellStyle.ALIGN_RIGHT:
                    ...
                    attrLayout.setAttribute(AttrIDConstant.PARA_INDENT_RIGHT_ID, Math.round(indent * KeyKt.PIXEL_TO_TWIPS));
                    AttrManage.instance().setParaHorizontalAlign(attr, WPAttrConstant.PARA_HOR_ALIGN_RIGHT);
                    break;
        ...
        if (pPr != null) {
            ...
            if (spcBef != null) {
                ...
                if (temp != null && temp.attribute("val") != null) {
                    ...
                    if (val != null && val.length() > 0) {
                        AttrManage.instance().setParaBefore(attr,
                                (int) (Integer.parseInt(val) / 100000.f * fontSize * POINT_PER_LINE_PER_FONTSIZE * KeyKt.POINT_TO_TWIPS));
                    }
                }
            }
