...
public abstract class AEventManage implements OnTouchListener,
                ...
                case MotionEvent.ACTION_UP:
                    if (!singleTabup) {
                        ...
                        if (Math.abs(initialYVelocity) > mMinimumVelocity
                            ...
                            if (!isScroll) {
                                isScroll = control.getApplicationType() == KeyKt.APPLICATION_TYPE_PPT;
                            }
                        ...
                        if (isScroll) {
                            isScroll = false;
                            if (control.getApplicationType() == KeyKt.APPLICATION_TYPE_WP && zoomChange) {
                                if (!control.getMainFrame().isZoomAfterLayoutForWord()) {
                                    control.actionEvent(EventConstant.APP_GENERATED_PICTURE_ID, null);
                                }
                            }
                            if (control.getApplicationType() == KeyKt.APPLICATION_TYPE_PPT) {
                        ...
                        if (control.getApplicationType() != KeyKt.APPLICATION_TYPE_WP) {
                            zoomChange = false;
                        }
                    }
    ...
    protected boolean zoom(MotionEvent event) {
        ...
        boolean isMinZoom = (int) (zoom * KeyKt.STANDARD_RATE) == (int) (fitZoom * KeyKt.STANDARD_RATE);
        ...
        if (zoomRateChanged) {
            isScroll = true;
            zoomChange = true;
            control.actionEvent(EventConstant.APP_ZOOM_ID, new int[]{(int) (zoom * KeyKt.STANDARD_RATE), midXDoublePoint, midYDoublePoint});
